...
export const Box = React.forwardRef(({ tag, ...props }, ref) => {
  return <StyledBox ref={ref} as={tag} {...props} />;
});
Box.defaultProps = {
  styleSheet: {},
};
...
export function Text({ tag, variant, styleSheet, ...props }) {
  ...
  const typographyVariantStyles =
    theme.typography.variants?.[variant] || theme.typography.variants.body2;
...
function parseCSS({ styleSheet }) {
  const statesSet = new Set(["hover", "focus", "disabled"]);
  ...
  function parser(styleSheet) {
    return (acc, styleKey) => {
      const styleKeyFormated = styleKey
        .split(/(?=[A-Z])/)
        .join("-")
        .toLowerCase();
